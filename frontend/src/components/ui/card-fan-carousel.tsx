...
export default function SocialCards({ cards }: SocialCardsProps) {
  ...
  const chevron = (direction: "left" | "right") => (
    <svg className="relative z-[2] w-4 h-4 md:w-5 md:h-5 text-black dark:text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
  ...
  return (
    <section className="flex flex-col items-center w-full py-4 lg:py-8 px-4 md:px-8 relative z-20">
      <style>{`
        ...
        @media (max-width: 480px) { .fan-layout { height: 18rem; } }
        @media (max-width: 640px) { .fan-layout { height: 21rem; } }
        @media (max-width: 768px) { .fan-layout { height: 23rem; } }
        @media (max-width: 1024px) { .fan-layout { height: 26rem; } }
        ...
        @media (max-width: 640px) {
          .fan-card { width: 7.5rem; height: 11.25rem; border-width: 2px; border-radius: 0.6rem; }
        }
        @media (max-width: 768px) {
          .fan-card { width: 9rem; height: 13.5rem; border-width: 2px; border-radius: 0.75rem; }
        }
        @media (max-width: 1024px) {
          .fan-card { width: 11.5rem; height: 17.25rem; border-width: 2px; border-radius: 0.9rem; }
        }
      `}</style>
      ...
      {needsPagination && (
        <div className="flex items-center justify-center gap-4 mt-4 md:mt-6 z-30">
          ...
          <div className="flex items-center gap-2">
            {cards.map((_, i) => (
              <span key={i} className={`w-2 h-2 rounded-full transition-all duration-300 ${i === centerIndex ? "bg-black/75 dark:bg-white/80 scale-[1.3]" : "bg-black/20 dark:bg-white/20"}`} />
            ))}
          </div>
