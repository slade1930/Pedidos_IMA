...
export default function UserDetailPage() {
  ...
  const handleEditSubmit = (data: any) => {
    updateMutation.mutate({ id: userId, data: data as AdminUpdateUserPayload });
  };
  ...
  if (isError || !user) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        ...
        <h2 className="text-lg font-semibold text-gray-900">Usuario no encontrado</h2>
        <p className="mt-2 text-sm text-gray-500">El usuario que buscas no existe o fue eliminado.</p>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <span className="text-sm text-gray-900 font-medium truncate">{user.full_name}</span>
      </div>

      <div className="max-w-2xl">
        <UserCard user={user} onEdit={handleEdit} onDelete={() => setShowDeleteConfirm(true)} />
      </div>
      ...
      {showEditModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-lg shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto p-6">
            ...
            <UserForm user={user} onSubmit={handleEditSubmit} onCancel={() => setShowEditModal(false)}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-lg shadow-xl w-full max-w-sm p-6">
            <div className="text-center">
              ...
              <h3 className="mt-4 text-lg font-semibold text-gray-900">Desactivar Usuario</h3>
              <p className="mt-2 text-sm text-gray-500">
                ¿Estás seguro de desactivar a <span className="font-medium text-gray-900">{user.full_name}</span>?
              </p>
            </div>
