// src/app/(dashboard)/fairs/[id]/page.tsx
...
import { FairCard } from "@/features/fairs/components/FairCard";
...
/**
 * Página de Detalle de Feria
 * 
 * Ruta: /dashboard/fairs/:id
...
export default function FairDetailPage() {
  ...
  const handleEditSubmit = (data: any) => {
    updateMutation.mutate({ id: fairId, data: data as UpdateFairPayload });
  };

  const handleDelete = () => {
    deleteMutation.mutate(fairId);
  };
  ...
  if (isError || !fair) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        ...
        <h2 className="text-lg font-semibold text-gray-900">Feria no encontrada</h2>
        <p className="mt-2 text-sm text-gray-500">La feria que buscas no existe o fue eliminada.</p>
        <button onClick={() => router.push("/dashboard/fairs")}
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <span className="text-sm text-gray-900 font-medium truncate">{fair.name}</span>
      </div>

      <div className="max-w-2xl">
        <FairCard fair={fair} onEdit={handleEdit} onDelete={() => setShowDeleteConfirm(true)} />
      </div>
      ...
      {showEditModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-lg shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6">
            ...
            <FairForm fair={fair} onSubmit={handleEditSubmit} onCancel={() => setShowEditModal(false)}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-lg shadow-xl w-full max-w-sm p-6">
            <div className="text-center">
              ...
              <h3 className="mt-4 text-lg font-semibold text-gray-900">Eliminar Feria</h3>
              <p className="mt-2 text-sm text-gray-500">
                ¿Estás seguro de eliminar <span className="font-medium text-gray-900">{fair.name}</span>?
              </p>
            </div>
